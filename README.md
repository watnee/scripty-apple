...
Every signature expires — after seven days with a free Apple ID, after a year
with a paid one — and the app is still on the Home Screen afterwards, it just
won't open. `install.sh` reads the real date out of the profile it installed,
tells you when it lands, and remembers it: run it again past that date and it
says up front that the copy has expired and that this run brings it back.

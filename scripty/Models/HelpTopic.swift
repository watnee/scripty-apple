        ...
        HelpSection(id: "documents", title: "Documents", topics: [
            HelpTopic(
                ...
                paragraphs: [
                    ...
                    + "plain song editors they sit where a screenplay keeps them — "
                    + "Undo in the bar at the top, greyed until there is something to "
                    + "take back, and Redo in the “…” beside it — with the same pair "
                    + "repeated at the left of the strip above the keyboard, under the "
                    + "thumb while the writing is happening. They walk the document "
                    + "itself, name and words alike: a burst of typing, a bullet the "
                    + "bar added, the title you just rewrote, an edit made offline, "
                    + "one step at a time and newest first — so taking back a rename "
                    + "never rewinds a paragraph you had finished with. That history "
                    + "stays on this device and starts fresh each time the document "
                    + "is opened.",
                    "A song with lyric lines keeps a longer memory, and draws both "
                    + "halves of the pair at the top of its editor. They step through "
                    + "the same history the browser shows while you have a connection; "
                    + "without one they step back through the lyric edits still "
                    + "waiting on this device, newest first, so a line typed offline "
                    + "can be taken back offline. Once those edits reach the server "
                    + "they become part of its history and undo goes back to walking "
                    + "that. Each step says what it did, the way a screenplay's does — "
                    + "a line that comes back may be one you cannot see from where you "
                    + "are. On either workspace screen the pair sits beside each title "
                    + "instead, since every song and every note keeps a history of its "
                    + "own.",

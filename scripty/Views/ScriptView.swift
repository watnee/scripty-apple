...
struct ScriptView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingSongs) {
            // Keyed on the document asked for so that a *second* widget row,
            // tapped while this sheet is already open, is not silently dropped.
            // `showingSongs` is already true by then, so nothing re-presents,
            // and the screen's starting list and document are fixed at
            // construction — a new identity is what rebuilds it and re-runs the
            // load that opens the named document. Nil for every other route, so
            // the toolbar button's sheet keeps its search and scroll position.

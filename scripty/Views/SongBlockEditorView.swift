...
struct SongBlockEditorView: View {
    ...
    private func lyricPosition(at offset: Int) -> (SongBlock, Int)? {
        ...
        for block in model.blocks {
            ...
            if offset <= start + length {
                return (block, characterOffset(in: text, utf16: offset - start))
            }
            start += length + 1
        }
    ...
    /// A UTF-16 offset into `text` as a count of Characters. Clamped rather
    /// than trusted: the offset was measured against the words the reader was
    /// showing, and a save landing in between can leave it past the end of the
    /// line it named.
    private func characterOffset(in text: String, utf16 offset: Int) -> Int {
        let bounded = max(0, min(offset, (text as NSString).length))
        guard let index = Range(NSRange(location: 0, length: bounded), in: text)?.upperBound
